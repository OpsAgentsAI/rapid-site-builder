...
const AGENT_ENGINE_RESOURCE = (process.env.AGENT_ENGINE_RESOURCE || '').trim();
const AGENT_ENGINE_LOCATION = (() => {
  const m = AGENT_ENGINE_RESOURCE.match(/\/locations\/([^/]+)\//);
  return (process.env.AGENT_ENGINE_LOCATION || (m && m[1]) || 'us-central1').trim();
})();
...
function buildEnginePhases(brief) {
  ...
  const schema = SPEC_SCHEMA +
    (he ? '. The brief is in Hebrew — write ALL copy in Hebrew for an Israeli audience and add "lang":"he","dir":"rtl"' : '');
  return [
    'We are building a one-page marketing website from this business brief. ' +
    ...
    (he ? ' (Hebrew first)' : '') + ', then to the SEO agent (Sam) for the SEO score. Do not publish.',
...
async function runBuild(brief, onStep) {
  ...
  if (!spec) {
    sink.allText = '';
    await engineTurn(token, 'Return ONLY the strict JSON site spec now — no prose.',
      sessionId, sink, phaseTimeout(), onStep ? (s) => onStep(s, phases.length) : null);
    spec = extractSpecFromText(sink.allText);
  }
  if (!spec) throw new Error('agent-engine produced no parseable site spec');
  return { spec, agentSteps: sink.agentSteps };
}
