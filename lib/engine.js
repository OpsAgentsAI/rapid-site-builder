...
function buildEnginePhases(brief) {
  ...
  // Design-trio turn (Dana UX, Remy visual, Kai frontend) is opt-in: the proxy
  // auto-deploys on merge while the engine is baked manually, so naming
  // sub-agents the pinned engine doesn't carry yet would break live builds.
  // Flip BUILDER_DESIGN_TRIO=1 only after an engine bake that includes them.
  if (process.env.BUILDER_DESIGN_TRIO === '1') {
    phases.push(
      'Continue: delegate to the UX agent (Dana) to review the page flow, then the visual ' +
      'agent (Remy) for the visual design system, then the frontend agent (Kai) for the ' +
      'pre-ship frontend check. Do not publish.'
    );
  }
  phases.push(
