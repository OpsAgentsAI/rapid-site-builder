...
async function publishOne(name, token, idx) {
  ...
  // The load-bearing XSS control is the magic-bytes gate above: a spoofed object
  // (e.g. HTML under image/png) is dropped before it can be served. We then pin
  // the canonical Content-Type (a REAL response header GCS serves from the fixed
  // field) and contentDisposition: inline, so the browser is told exactly what
  // the body is and an explicitly-typed image/* body is never sniffed into HTML.
  // NOTE: a true `X-Content-Type-Options: nosniff` RESPONSE header cannot be set
  // on a direct storage.googleapis.com object — custom metadata.metadata entries
  // surface only as `x-goog-meta-*`, which browsers ignore. The breadcrumb below
  // documents intent; delivering a real nosniff header needs a CDN/LB in front of
  // the public bucket (tracked as a follow-up card). The magic-bytes gate + real
  // Content-Type already close the stored-XSS vector without it.
  const saveOpts = {
