...
function fonts(spec) {
  // Hebrew specs load Hebrew-coverage families — Fraunces/Sora/Inter carry no
  // Hebrew glyphs, so a lang=he site would render headlines in the bare system
  // fallback. Frank Ruhl Libre (serif vibes) / Rubik + Heebo cover Hebrew+Latin.
  const fam = spec.lang === 'he'
    ? 'family=Frank+Ruhl+Libre:wght@500;600;700&family=Rubik:wght@500;600;700;800&family=Heebo:wght@400;500;600;700'
    : 'family=Fraunces:opsz,wght@9..144,500;9..144,640;9..144,720&family=Sora:wght@500;600;700;800&family=Inter:wght@400;500;600;700';
...
function headFont(spec) {
  if (spec.lang === 'he') return SERIF_VIBES.has(spec.vibe) ? "'Frank Ruhl Libre',Georgia,serif" : "'Rubik',system-ui,sans-serif";
  return SERIF_VIBES.has(spec.vibe) ? "'Fraunces',Georgia,serif" : "'Sora',system-ui,sans-serif";
}
function bodyFont(spec) {
  return spec.lang === 'he' ? "'Heebo',system-ui,sans-serif" : "'Inter',system-ui,sans-serif";
}
// Chrome strings — the fixed UI text around the agent-generated copy (nav,
// section eyebrows, contact labels, CTA defaults, footer provenance). A Hebrew
// spec (lang:'he', stamped by the crew / fallbackSpec) gets the Hebrew chrome;
// everything else stays English. Business-to-visitor voice is masculine plural
// per the MSApps Hebrew house style (the feminine voice is reserved for the
// builder app shell, which addresses the operator directly).
const HE_CHROME = {
  'Main': 'ראשי',
  'Get in touch': 'דברו איתנו',
  'What we offer': 'מה אנחנו מציעים',
  'Why us': 'למה אנחנו',
  'Why choose us': 'למה לבחור בנו',
  'About': 'אודות',
  'Our story': 'הסיפור שלנו',
  'Find us': 'איפה מוצאים אותנו',
  'Address': 'כתובת',
  'Phone': 'טלפון',
  'Email': 'אימייל',
  'Opening hours': 'שעות פתיחה',
  'Come by': 'בואו לבקר',
  'Visit & contact': 'ביקור ויצירת קשר',
  'Visit': 'ביקור',
  'Hours': 'שעות פתיחה',
  'Gallery': 'גלריה',
  'A look inside': 'הצצה פנימה',
  'Ready when you are': 'מחכים רק לכם',
  'Contact us': 'צרו קשר',
  ...
  'Menu': 'תפריט',
  'See the menu': 'לתפריט',
  'See our services': 'לשירותים שלנו',
  'Book an appointment': 'קביעת תור',
  'photo': 'תמונה',
  'Designed, written & shipped by an AI agent team · Rapid Site Builder demo':
    'עוצב, נכתב ושוגר על ידי צוות סוכני AI · הדגמת Rapid Site Builder'
};
function L(spec, en) { return (spec && spec.lang === 'he' && HE_CHROME[en]) || en; }
...
.nav .btn{padding:11px 22px;font-size:14px;min-height:44px}
...
.vrow a{display:inline-block;padding-block:10px;margin-block:-10px}
...
.gallery .gcell{width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:18px;background:var(--bg2);border:1px solid color-mix(in srgb,var(--ink) 8%,transparent)}
...
// The visitor's own photos/videos, server-verified and copied into our public
// bucket before they ever reach this function. Plain <img>/<video> tags only —
// the published page stays script-free by construction.
function gallerySection(spec, media) {
  if (!media || !media.length) return '';
  const cells = media.map((m, i) => m.kind === 'video'
    ? `<video class="gcell" controls preload="metadata" src="${m.url}"></video>`
    : `<img class="gcell" loading="lazy" alt="${esc(spec.business)} — ${L(spec, 'photo')} ${i + 1}" src="${m.url}"/>`).join('\n    ');
  return `<section id="gallery" class="tint"><div class="wrap">
  ${secHead(spec, L(spec, 'Gallery'), L(spec, 'A look inside'), '', true)}
  <div class="gallery">${cells}</div></div></section>`;
}
...
// ---- layouts ----------------------------------------------------------------------
function layoutStandard(spec, img, gallery) {
  return navBar(spec, [
    ...
    + whySection(spec, false, false) + gallery + visitSection(spec, false) + ctaSection(spec) + `</main>` + footerBlock(spec);
}
function layoutServices(spec, img, gallery) {
  return navBar(spec, [
    ...
    + itemsSection(spec, 2, false) + whySection(spec, true, true) + gallery + aboutSection(spec) + ctaSection(spec) + `</main>` + footerBlock(spec);
}
function layoutCatalog(spec, img, gallery) {
  return navBar(spec, [
    { id: 'items', label: spec.items_heading || L(spec, 'Menu') },
    { id: 'visit', label: L(spec, 'Visit') },
    { id: 'about', label: L(spec, 'About') }
  ]) + `<main id="top">` + heroCover(spec, img, '#items', spec.items_heading || L(spec, 'See the menu'))
    + itemsSection(spec, 3, false) + gallery + aboutSection(spec) + visitSection(spec, false) + ctaSection(spec) + `</main>` + footerBlock(spec);
}
function layoutBooking(spec, img, gallery) {
  ...
  return navBar(spec, [
    ...
    + whySection(spec, false, false) + itemsSection(spec, 2, true) + gallery + visitSection(spec, true) + ctaSection(spec) + `</main>` + footerBlock(spec);
}
...
function render(spec, { heroImage, userMedia } = {}) {
  const img = escUrl(heroImage);
  const media = (Array.isArray(userMedia) ? userMedia : [])
    .map(m => ({ url: escUrl(m && m.url), kind: m && m.kind === 'video' ? 'video' : 'image' }))
    .filter(m => m.url)
    .slice(0, 8);
  const lang = spec.lang === 'he' ? 'he' : 'en';
  const dir = (spec.dir === 'rtl' || lang === 'he') ? ' dir="rtl"' : '';
  const body = (LAYOUTS[spec.layout] || LAYOUTS.standard)(spec, img, gallerySection(spec, media));
