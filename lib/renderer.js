...
function headFont(spec) {
  if (spec.lang === 'he') return "'Frank Ruhl Libre',serif";
...
// chrome-string localizer (template strings only — copy comes from the spec)
function L(spec, en, he) { return spec.lang === 'he' ? he : en; }
...
function navBar(spec, links) {
  ...
  <nav aria-label="${L(spec, 'Main', 'ראשי')}">${ls}<a class="btn" href="#cta">${esc(spec.cta_button || L(spec, 'Get in touch', 'דברו איתנו'))}</a></nav>
</div></header>`;
}
...
function itemsSection(spec, cols, tint) {
  ...
  ${secHead(spec, L(spec, 'What we offer', 'מה תמצאו אצלנו'), spec.items_heading || L(spec, 'What we offer', 'מה תמצאו אצלנו'), '', true)}
...
function whySection(spec, tint, numbered) {
  ...
  ${secHead(spec, L(spec, 'Why us', 'למה אנחנו'), spec.why_heading || L(spec, 'Why choose us', 'למה לבחור בנו'), '', true)}
  <div class="grid">${cards}</div></div></section>`;
}
function aboutSection(spec) {
  return `<section id="about"><div class="wrap prose">
  <span class="k" style="display:block;color:var(--accent);font-weight:700;text-transform:uppercase;letter-spacing:.12em;font-size:12.5px;margin-bottom:12px">${L(spec, 'About', 'עלינו')}</span>
  <h2>${esc(spec.about_heading || L(spec, 'Our story', 'הסיפור שלנו'))}</h2>
...
function visitSection(spec, hoursFirst) {
  ...
  const contactCard = `<div class="visit-card">
    <h3>${L(spec, 'Find us', 'איפה אנחנו')}</h3>
    ${c.address ? `<div class="vrow"><span class="vi" aria-hidden="true">📍</span><div><b>${L(spec, 'Address', 'כתובת')}</b>${esc(c.address)}</div></div>` : ''}
    ${c.phone ? `<div class="vrow"><span class="vi" aria-hidden="true">📞</span><div><b>${L(spec, 'Phone', 'טלפון')}</b><a href="tel:${esc(String(c.phone).replace(/[^\d+]/g, ''))}">${esc(c.phone)}</a></div></div>` : ''}
    ${c.email ? `<div class="vrow"><span class="vi" aria-hidden="true">✉️</span><div><b>${L(spec, 'Email', 'אימייל')}</b><a href="mailto:${esc(c.email)}">${esc(c.email)}</a></div></div>` : ''}
  </div>`;
  const hoursCard = `<div class="visit-card">
    <h3>${L(spec, 'Opening hours', 'שעות פתיחה')}</h3>
    <p class="hours">${esc(c.hours || '')}</p>
  </div>`;
  return `<section id="visit"><div class="wrap">
  ${secHead(spec, L(spec, 'Come by', 'בואו לבקר'), L(spec, 'Visit & contact', 'ביקור ויצירת קשר'), '', true)}
...
function ctaSection(spec) {
  ...
  <h2>${esc(spec.cta_heading || L(spec, 'Ready when you are', 'מחכים לכם'))}</h2>
  <p>${esc(spec.cta_text || '')}</p>
  <a class="btn" href="${c.email ? 'mailto:' + esc(c.email) : '#visit'}">${esc(spec.cta_button || L(spec, 'Contact us', 'צרו קשר'))}</a>
</div></div></section>`;
}
function footerBlock(spec) {
  ...
  <div>${esc([c.address, c.phone].filter(Boolean).join(' · '))}</div>
</div><div class="prov">${L(spec, 'Designed, written & shipped by an AI agent team · Rapid Site Builder demo', 'עוצב, נכתב ושוגר על־ידי צוות סוכני AI · דמו Rapid Site Builder')}</div></footer>`;
}

// ---- heroes ----------------------------------------------------------------------
function heroSplit(spec, img) {
  const media = img
    ? `<div class="hero-media" role="img" aria-label="${esc(spec.business)}" style="background-image:url('${img}')"><span class="tag">${esc(spec.tagline)}</span></div>`
    : `<div class="hero-art" aria-hidden="true"></div>`;
  return `<div class="hero"><div class="wrap hero-grid">
  <div><span class="eyebrow">${esc(spec.tagline)}</span>
    <h1>${esc(spec.business)}</h1>
    <p class="lead">${esc(spec.about)}</p>
    <div class="hero-cta"><a class="btn" href="#cta">${esc(spec.cta_button || L(spec, 'Get started', 'בואו נתחיל'))}</a><a class="btn ghost" href="#items">${esc(spec.items_heading || L(spec, 'See more', 'גלו עוד'))}</a></div>
  </div>${media}</div></div>`;
}
function heroCover(spec, img, ctaHref, ctaLabel) {
  ...
  <span class="eyebrow">${esc(spec.tagline)}</span>
  <h1>${esc(spec.business)}</h1>
  <p class="lead">${esc(spec.about)}</p>
...
function heroCenter(spec, ctaHref, ctaLabel, trustline) {
  ...
  <h1>${esc(spec.business)}</h1>
  <p class="lead">${esc(spec.about)}</p>
  <div class="hero-cta"><a class="btn" href="${ctaHref}">${esc(ctaLabel)}</a><a class="btn ghost" href="#items">${esc(spec.items_heading || L(spec, 'Explore', 'גלו עוד'))}</a></div>
  ${trustline}
</div></div>`;
}
...
function layoutStandard(spec, img) {
  return navBar(spec, [
    { id: 'about', label: L(spec, 'About', 'עלינו') },
    { id: 'items', label: spec.items_heading || L(spec, 'Offerings', 'השירותים') },
    { id: 'visit', label: L(spec, 'Visit', 'בואו לבקר') }
...
function layoutServices(spec, img) {
  return navBar(spec, [
    { id: 'items', label: spec.items_heading || L(spec, 'Services', 'שירותים') },
    { id: 'why', label: L(spec, 'Why us', 'למה אנחנו') },
    { id: 'about', label: L(spec, 'About', 'עלינו') }
  ]) + `<main id="top">` + heroCover(spec, img, '#items', spec.cta_button || L(spec, 'See our services', 'לשירותים שלנו'))
...
function layoutCatalog(spec, img) {
  return navBar(spec, [
    { id: 'items', label: spec.items_heading || L(spec, 'Menu', 'תפריט') },
    { id: 'visit', label: L(spec, 'Visit', 'בואו לבקר') },
    { id: 'about', label: L(spec, 'About', 'עלינו') }
  ]) + `<main id="top">` + heroCover(spec, img, '#items', spec.items_heading || L(spec, 'See the menu', 'לתפריט'))
...
function layoutBooking(spec, img) {
  ...
  return navBar(spec, [
    { id: 'why', label: L(spec, 'Why us', 'למה אנחנו') },
    { id: 'items', label: spec.items_heading || L(spec, 'Services', 'שירותים') },
    { id: 'visit', label: L(spec, 'Hours', 'שעות') }
  ]) + `<main id="top">` + heroCenter(spec, '#cta', spec.cta_button || L(spec, 'Book an appointment', 'לקביעת תור'),
