...
// Provider dispatch (card Azz8fInK): managed env by default, per-tenant BYOK
// when injected. With nothing injected these are exactly the prior env reads.
const _image = resolveProviderConfig().image;
const BUCKET = _image.bucket;
const IMAGE_PROJECT = _image.project;
const IMAGE_REGION = _image.region;
const IMAGE_MODEL = _image.model;
...
async function generatePng(category, style) {
  ...
  // Gemini 3 image models serve from the global endpoint (no region prefix).
  const host = IMAGE_REGION === 'global'
    ? 'https://aiplatform.googleapis.com/v1beta1'
    : `https://${IMAGE_REGION}-aiplatform.googleapis.com/v1`;
  const url = `${host}/projects/${IMAGE_PROJECT}/locations/${IMAGE_REGION}/publishers/google/models/${IMAGE_MODEL}:generateContent`;
