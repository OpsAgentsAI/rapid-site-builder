...
// directly; `limit` bounds both the response and the number of meta downloads
// so a large bucket can't unbound an admin page load. Admin-only at the route.
async function listAllSites(limit = 500) {
  if (!BUCKET) return [];
  const [files] = await storage.bucket(BUCKET).getFiles({ prefix: 'sites/', maxResults: 5000 });
  const metas = files.filter(f => /^sites\/[a-f0-9]{8}\/meta\.json$/.test(f.name));
  const out = [];
  for (const f of metas) {
    ...
    } catch { /* one unreadable meta must not break the admin listing */ }
  }
