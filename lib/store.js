...
// Tenant entitlement records (card KY2YTmoL) — tenants/{uid}.json in the same
// bucket, mirroring the owners/{uid}/ pattern. Firebase uids are url-safe
// (≤128 chars); the regex gate means no caller input ever shapes a path.
const UID_RE = /^[A-Za-z0-9_-]{1,128}$/;

async function loadTenant(uid) {
  if (!BUCKET || !UID_RE.test(String(uid))) return null;
  try {
    const [buf] = await storage.bucket(BUCKET).file(`tenants/${uid}.json`).download();
    return JSON.parse(buf.toString('utf8'));
  } catch {
    return null; // no record yet (the common case) — caller defaults the plan
  }
}

async function saveTenant(uid, record) {
  if (!BUCKET) throw new Error('PUBLISHED_SITES_BUCKET not configured');
  if (!UID_RE.test(String(uid))) throw new Error('Bad uid');
  await storage.bucket(BUCKET).file(`tenants/${uid}.json`).save(JSON.stringify(record), {
    contentType: 'application/json', resumable: false
  });
}

module.exports = { saveSite, loadSite, rememberDeviceSite, listDeviceSites, saveLlms, loadLlms, listSitesByOwner, loadTenant, saveTenant, UID_RE };
