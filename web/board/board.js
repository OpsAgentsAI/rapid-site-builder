...
(() => {
  ...
  function renderTeam() {
    ...
    const cards = ROSTER.map(a => {
      ...
      return `<article class="card" draggable="true" data-id="${esc(a.id)}" data-fstatus="${esc(fstatus)}" data-name="${esc(a.name)}" data-role="${esc(a.role)}" style="--hue:var(--agent-${a.color})">
