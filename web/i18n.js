...
(function () {
  const STRINGS = {
    en: {
      ...
      // build-feed runtime chrome (previously hardcoded English in index.html)
      'feed.brings': '{a} brings in {b}',
      'feed.working': 'working: {tool}',
      'feed.details': 'details',
      'feed.results': 'shared structured results',
      'media.uploading': 'uploading…',
      'media.ready.photo': '📷 ready',
      'media.ready.video': '🎞 ready',
      'media.aria': 'Add photos or videos',
      'dyn.shipped.pill': 'shipped {url}',
      'dyn.shipped.line': 'Shipped the site live: {url}',
      'feed.max.0': 'render(spec) → single-file HTML',
      'feed.max.1': 'saved to site store',
      'feed.max.2': 'live at {url}',
      ...
      'board.filter.toolbar': 'Filter the team roster',
    ...
    he: {
      ...
      // build-feed runtime chrome (previously hardcoded English in index.html)
      'feed.brings': 'המקל עובר מ{a} אל {b}',
      'feed.working': 'בעבודה: {tool}',
      'feed.details': 'פרטים',
      'feed.results': 'שיתוף תוצאות מובנות',
      'media.uploading': 'מעלה…',
      'media.ready.photo': '📷 מוכן',
      'media.ready.video': '🎞 מוכן',
      'media.aria': 'הוספת תמונות או סרטונים',
      'dyn.shipped.pill': 'שוגר {url}',
      'dyn.shipped.line': 'האתר שוגר לאוויר: {url}',
      'feed.max.0': 'render(spec) → קובץ HTML יחיד',
      'feed.max.1': 'נשמר במאגר האתרים',
      'feed.max.2': 'באוויר: {url}',
      // ===== /board operate console =====
      ...
      'board.filter.toolbar': 'סינון סגל הצוות',
